...
int main(int argc, char *argv[])
{
    ...
    std::cout << "discovering ancestors" << std::endl;

    //  Make a reversed version of the digraph created above
    Graph rg;
    boost::copy_graph(boost::make_reverse_graph(g), rg);

    //  Find ancestors of the "shiny gold" vertex
    //  Note that since the graph is reversed, descendants are actually ancestors
    VertexIterator sg = findVertex("shiny gold", g);
    std::vector<Vertex> ancestors = findDescendants(*sg, rg);

    //  Use a set to remove duplicate ancestors
    std::set<std::string> uniqueAncestors;
    for(std::vector<Vertex>::const_iterator it=ancestors.begin(); it!=ancestors.end(); ++it)
    {
        uniqueAncestors.insert(it->name);
    }

    std::cout << "unique ancestors: " << uniqueAncestors.size() << std::endl;
